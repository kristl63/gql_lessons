...
async def planned_lesson_page(
    self, info: strawberry.types.Info, skip: int = 0, limit: int = 10, where: Optional[PlannedLessonInputFilter] = None
) -> List[PlannedLessonGQLModel]:
...
class PlannedLessonInsertGQLModel:
    ...
    rbacobject: strawberry.Private[IDType]
...
async def planned_lesson_insert(self, info: strawberry.types.Info, lesson: PlannedLessonInsertGQLModel) -> PlannedLessonResultGQLModel:
    return encapsulateInsert(info, PlannedLessonGQLModel.getLoader(info), lesson, PlannedLessonResultGQLModel(msg="ok", id=None))
...
async def planned_lesson_update(self, info: strawberry.types.Info, lesson: PlannedLessonUpdateGQLModel) -> PlannedLessonResultGQLModel:
    return encapsulateUpdate(info, PlannedLessonGQLModel.getLoader(info), lesson, PlannedLessonResultGQLModel(msg="ok", id=None))
